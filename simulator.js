...
function ensureRegistry(){
 ...
 let changed=false;
 items=items.map((p,i)=>{
   if(!p.id){p.id=`pf-${Date.now()}-${i}`;changed=true}
   const capital=Number(p.capital);
   const start=Number(p.start);
   if(!Number.isFinite(start)||start<=0){
     p.start=(Number.isFinite(capital)&&capital>0)?capital:10000;
     changed=true;
   }
...
function loadState(){
 ...
 const safeStart=(Number(pf?.start)>0)?Number(pf.start):10000;
 if(!s||s.version!==14)s=defaultState(safeStart);

 s.positions||={};s.openOrders||=[];s.transactions||=[];s.events||=[];s.appliedEvents||={};s.market||={};

 const emptyAccount =
   Number(s.cash||0)===0 &&
   Number(s.startingEquity||0)===0 &&
   Number(s.loan||0)===0 &&
...
(function simulatorStartupCheck(){
 const s=loadState();
 const pf=activePortfolio();
 const assetSelect=document.getElementById("trade-asset");
 const portfolioSelect=document.getElementById("portfolio-select");
 if(!assetSelect?.options.length || !portfolioSelect?.options.length){
   msg("Simulator startup issue detected. Reload once; if it persists, use this clean v1.5.2 build without merging simulator files.","bad");
   return;
 }
 if(!assetSelect.value){ assetSelect.value="AUR"; selectedSymbol="AUR"; renderAll(); }
 if(!portfolioSelect.value && pf){ portfolioSelect.value=pf.id; }
})();
