...
document.getElementById("replay-days").oninput=renderReplay;
document.getElementById("replay-start").onclick=()=>{const s=loadState(),days=+document.getElementById("replay-days").value,index=Math.max(30,s.market[selectedSymbol].length-1-days);s.replay={active:true,index,account:{cash:10000,startingEquity:10000,loan:0,interestAccrued:0,realizedPnL:0,positions:{},openOrders:[],transactions:[],displayHistoryClearedAt:0}};saveState(s);renderAll();msg("Historical Replay started. Future prices are hidden.","good")};
document.getElementById("replay-next").onclick=()=>{const s=loadState();if(!inReplay(s))return;if(s.replay.index<s.market[selectedSymbol].length-1){s.replay.index++;processOpenOrders(s);saveState(s);renderAll()}else msg("Replay has reached the current simulated date.","good")};
document.getElementById("replay-exit").onclick=()=>{const s=loadState();s.replay=null;saveState(s);renderAll();msg("Returned to live simulated portfolio.","good")};
sideUI();renderAll();
