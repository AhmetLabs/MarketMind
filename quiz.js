...
next.onclick=()=>{if(i<quiz.length-1){i++;render()}else{q.textContent=`Quiz complete: ${score}/${quiz.length}`;grid.innerHTML="";next.classList.add("hidden");const s=getState();s.coins+=score*4;saveState(s)}};
render();
